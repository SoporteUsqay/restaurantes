        ...
        <div class="row-border">
            <div class="col-lg-12">
                <?php
                    ...
                    $total_descuentos = 0;
                    $total_ventas = 0; 
                    $query_mes = "Select fechaCierre, count(*) as pedidos, sum(total) as vendido, sum(descuento) as descuentos from pedido where estado = 1 AND month(fechaCierre) = '".$mes."' AND year(fechaCierre) = '".$anio."' group by fechaCierre";
                        ...
                        <table class='tb display' cellspacing='0' width='100%' border='0'>
                            <thead>
                                <tr>
                                    <th>Día</th>
                                    <th># Pedidos</th>
                                    <th>Descuentos(<?php echo $r_moneda["simbolo"];?>)</th>
                                    <th>Total Vendido(<?php echo $r_moneda["simbolo"];?>)</th>
                                </tr>
                            </thead>
                            <tbody >
                            <?php 
                            if(is_array($resultado)){
                                foreach($resultado as $res){
                                    ...
                                    echo "<td>".intval($res["pedidos"])."</td>";
                                    echo "<td>".round(floatval($res["descuentos"]),2)."</td>";
                                    echo "<td>".round(floatval($res["vendido"]),2)."</td>";
                                    echo "</tr>";

                                    $total_pedidos = $total_pedidos + intval($res["pedidos"]);
                                    $total_descuentos = $total_descuentos + round(floatval($res["descuentos"]),2);
                                    $total_ventas = $total_ventas + round(floatval($res["vendido"]),2);
                                }

                                echo "<tr class='table-danger'>";
                                echo "<td class='celeste'></td>";
                                echo "<td class='celeste'>".$total_pedidos."</td>";
                                echo "<td class='celeste'>".$total_descuentos."</td>";
                                echo "<td class='celeste'>".$total_ventas."</td>";
                                echo "</tr>";
                            }
                            ?>
                            </tbody>
