<?php
    ...
    $titulo_importante = "Reporte de Creditos del ";
            ...
            <div class="col-lg-12">
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h4><i class="glyphicon glyphicon-list-alt"></i> Cuentas Pendientes (CRÉDITOS)</h4>
                    </div>
                ...
                <table id="tblCuentasPorPagar" class="table table-borderer">
                    ...
                    <tbody>
                        ...
                        if(is_null($cliente)){
                            ...
                            inner join mesas m on m.pkMesa=p.pkMesa where p.estado=4 and DATE_FORMAT(fechaApertura,'%Y-%m-%d') between '$fechaInicio' and '$fechaFin' group by p.pkPediido;";
                        }else{
                            $query = "SELECT *,
                            case when (tipo_cliente = 1 AND character_length(p.documento) > 0 AND character_length(p.documento) < 8) then 
                            (select nombre from cliente_generico where id=p.documento)
                            ...
                            inner join trabajador t
                            on d.pkMozo=t.pkTrabajador
                            inner join mesas m on m.pkMesa=p.pkMesa where p.estado=4 and DATE_FORMAT(fechaApertura,'%Y-%m-%d') between '$fechaInicio' and '$fechaFin' AND p.documento = '$cliente' group by p.pkPediido;";
                        }
                        $result = $db->executeQuery($query);
                        while ($row = $db->fecth_array($result)) {
                            ...
                            echo "<td>";
                            echo $row['total'];
                            $suma_total = $suma_total + floatval($row["total"]);
                            echo "</td>";
                            echo "<td>";
                            echo $row['nmesa'];
                            echo "</td>";
                            ...
                            echo "</td>";
                            echo "<td>";
                            echo "<a href='javascript:void(0)' onclick='VerPedidoPorPagar(\"" . $row['pkPediido'] . "\"," . $row['pkMesa'] . ")'>Ver Pedido</a>";
                            echo "</td>";
                            ?>
                            </td>
                            </tr>
    ...
    <script type="text/javascript" src="Application/Views/Sale/js/CuentasPendientes.js.php"></script>
    ...
    <script>
        ...
        function VerPedidoPorPagar($pedido, $mesa) {
            window.open('<?php echo Class_config::get('urlApp') ?>/?controller=Pedidos&action=ShowPorPagar&pkPedido=' + $pedido + '&pkmesa=' + $mesa, '_blank');
        }
