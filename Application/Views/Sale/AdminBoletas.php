<?php
    ...
    $codComprobante = "";
    if (isset($_REQUEST['cod_comprobante'])){
        $codComprobante = $_REQUEST['cod_comprobante'];
    }
    
    $titulo_importante = "Consolidado de Boletas ";
...
<link rel="stylesheet" href="reportes/recursos/js/plugins/tablas/jquery.dataTables.min.css">
    ...
    <div class="row usqay-bdy">            
            ...
            <div class="panel panel-primary" id='pfecha'>
                ...
                <div class="panel-body row">
                    ...
                    <div class='control-group col-md-2'>        
                        <label> </label>
                        <p>
                            <button type='button' class='btn btn-primary' onclick='busquedaBoletas()'>Filtrar</button>
                        </p>
                    </div>
                </div>
            </div>                    
        ...
        <div class="row">
            ...
            <table id="tbl_boleta" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th></th>
                        <th></th>
                ...
                <tbody>
                    ...
                    //Obtenemos series
                    $serie_boleta = "";
                    $query_s = "Select * from cloud_config where parametro = 'sboleta'";
                    $result_s = $db->executeQuery($query_s);
                    if($row_s = $db->fecth_array($result_s)){
                        $serie_boleta = $row_s["valor"];
                    }

                    //Obtenemos datos de comprobantes
                    ...
                    "WHEN (character_length(c.documento) = 8) THEN ".
                    "(SELECT CONCAT(Nombres) FROM person p WHERE p.documento = c.documento) ".
                    "WHEN (character_length(c.documento) > 0 AND character_length(c.documento) < 8) THEN ".
                    "(SELECT cg.nombre FROM cliente_generico cg WHERE cg.id = c.documento) ".
                    "WHEN (character_length(c.documento) = 0) THEN ".
                    "(SELECT '-') ".
                    "END AS cliente ".
                    "FROM comprobante c ".
                    "WHERE c.fecha  BETWEEN '$fechaInicio' AND '$fechaFin' AND ".
                    "c.pkTipoComprobante = 1 ".
                    ...
                    while ($row = $db->fecth_array($result)) {
                        ...
                        echo "<a class='btn' title='Re Imprimir' onclick='imprimirBoleta(\"" . $row['pkComprobante'] . "\")'><span class='glyphicon glyphicon-print'></span></a>";
                        echo "</td>";
                        echo "<td class='text-center'> ";
                        echo "<a class='$grilla' title='Anular' onclick='anularBoleta(\"" . $row['pkComprobante'] . "\")'><span class='glyphicon glyphicon-remove-circle'></span></a>";
                        echo "</td>";
                        echo "<td class='text-center'>" . $serie_boleta . "</td>";
                        echo "<td class='text-center'>" . str_pad($row['ncomprobante'], 8, "0", STR_PAD_LEFT) . "</td>";
                        echo "<td class='text-center'>03</td>";
...
<script>
    ...
    function imprimirBoleta($codComprobante) {
        var consumo = 1;
        var param2 = {'pkPedido': $codComprobante, 'terminal': '<?php echo $_COOKIE['t'] ?>', 'tipo': 'BOLETA', 'aux': '<?php echo UserLogin::get_id();?>,'+consumo};
    $.ajax({
    ...
    function busquedaBoletas(){
        window.location.href="<?php echo Class_config::get('urlApp') ?>/?controller=Sale&action=ShowBoleta&" +$('#frmFiltroBoletas').serialize();
    }
</script>