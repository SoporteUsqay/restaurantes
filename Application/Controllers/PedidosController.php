...
class Application_Controllers_PedidosController {
    ...
    public function __construct($action) {
        ...
        switch ($action) {
            ...
            case 'ListProductAccesoRapidoAction':
                $this->_listProductAccesoRapido();
                break;

            case 'saveCambioDePlatoAction':
                $this->_saveCambioDePlato();
                break;
        
        }
    }

    public function _saveCambioDePlato()
    {
        $lista = $_REQUEST['platos'];
        ...
        foreach ($lista as $index => $item) {
            ...
            if ($index == 0) {
                $query = "select * from detallepedido where pkDetallePedido = ${item['pkDet']}";
                ...
                $query = "update detallepedido set estado = 3, descripcionPedido = 'is-for-change' where pkDetallePedido = ${item['pkDet']}";

                $db->executeQueryEx($query);
            }

            $query = "insert into detallepedido (cantidad, precio, mensaje, estado, pkPediido, fechaPedido, horaPedido, horaTermino, pkPlato, pkProducto, pkCocinero, pkMozo, estadoImpresion, impresopor, descripcionPedido, pkPromocion, tipoPedido, terminal) values ";

            $query .= "(${item['caN']}, ${item['preN']}, null, 1, ${detalle['pkPediido']}, null, now(), now(), '${item['pkpN']}', null, ${detalle['pkCocinero']}, ${detalle['pkMozo']}, 0, null, 'cambio-${item['pkDet']}', ${detalle['pkPromocion']}, ${detalle['tipoPedido']}, '$terminal')";

            $db->executeQueryEx($query);

            $res = $db->executeQueryEx("SELECT LAST_INSERT_ID() as id_detalle");
            ...
            $query = "insert into accion_caja (pk_accion, tipo_accion, caja) values ";

            $query .= "($id_detalle, 'DET', '".$_COOKIE['c']."')";
    ...
    private function _obtienePedidosPantalla(){
        ...
        nombre_trabajador, m.nmesa as mesa, s.nombre as salon, s.pkSalon as pkSalon, 
        p.npersonas as personas, dp.estado as estado, pcli.nombre_cliente as cliente,
        cliext.nombres_y_apellidos as cliente_ext
        FROM detallepedido dp LEFT JOIN pedido_cliente pcli ON pcli.pkPediido = dp.pkPediido
        LEFT JOIN cliente_externo cliext ON cliext.id_pedido = dp.pkPediido, pedido p, plato pl, trabajador tr, mesas m, salon s
        WHERE dp.pkMozo = tr.pkTrabajador AND 
        dp.pkPlato = pl.pkPlato AND dp.pkPediido = p.pkPediido AND p.pkMesa = m.pkMesa AND 
        m.pkSalon = s.pkSalon AND p.estado in(0, 1, 2) AND DATE(p.fechaCierre) = DATE((select fecha from cierrediario limit 1))".$consulta_tipos;
        $resultado = array();
        ...
        while ($row = $db->fecth_array($result)) {
            ...
            $tmp["cliente"] = $row["cliente"];
            $tmp["cliente_ext"] = $row["cliente_ext"];
            $resultado[] = $tmp;
        }
        ...
        if (self::$session->validateStartSesion()) {
            if (!self::$session->validateSesion()) {
                ...
                if($row_abierto = $db->fecth_array($r_abierto)){
                    ...
                    # enviar solo cuando tenga detalles
                    if ($lista_detalles != "<ul></ul>") {
    ...
    private function _listProductAccesoRapido() {
        if (self::$session->validateStartSesion()) {
            if (!self::$session->validateSesion()) {
                $objViewPedidos = new Application_Models_ProductosModel();
                $tipo = "";
                ...
                $hace = date("Y-m-d", strtotime(date('Y-m-d') . " - 1 month"));

                $db = new SuperDataBase();

                $query = "SELECT
                    pkPlato,
                    count(pkPlato) AS cantidad
                FROM
                    detallepedido
                WHERE
	                horaPedido >= '$hace'    
                ...
                while ($row = $db->fecth_array($res)) {
                    $acceso_rapido[] = "'" . $row['pkPlato'] . "'";
                }

                $acceso_rapido = implode(', ', $acceso_rapido);

                $objViewPedidos->listAllProduct($tipo, $page, $size, "($acceso_rapido)");
            } else {
                self::$session->redirect();
            }
        } else {
    ...
    private function _cancelaPedidoCredito() {
        if (self::$session->validateStartSesion()) {
            if (!self::$session->validateSesion()) {
                ...
                $objModelPedidos->CancelaPedidoCredito($_POST['pkPedido'], $_REQUEST['tipo_cliente'], $documento, $_REQUEST['total'], $_REQUEST['comentario']);
            } else {
                self::$session->redirect();
            }
        } else {
    ...
    private function _cancelaPedidoACuenta() {
        if (self::$session->validateStartSesion()) {
            if (!self::$session->validateSesion()) {
                ...
                $objModelPedidos->CancelaPedidoACuenta($_POST['pkPedido'], $_REQUEST['tipo_cliente'], $documento, $_REQUEST['total'], $_REQUEST['comentario']);
            } else {
                self::$session->redirect();
            }
        } else {
