...
class Application_Controllers_CajaController {
    ...
    private function _ListCortes(){
        $db = new SuperDataBase();
        $array = array();
        $query="Select c.* from corte c, accion_caja ac where c.fecha_cierre = '".$_REQUEST["fecha"]."' AND ac.pk_accion = c.id AND ac.tipo_accion = 'CUT' AND ac.caja = '".$_REQUEST["caja"]."' order by c.id DESC";
