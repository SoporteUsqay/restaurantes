...
<script>
    ...
    $(document).ready(function() {
        ...
        //PARA LAS TABLES GG
        if (localStorage.getItem("version_pantalla") == null || localStorage.getItem("version_pantalla") != "1") {
            localStorage.clear();
            localStorage.setItem("version_pantalla", "1");
        }
    ...
    function cargarTipos() {
        ...
        $.ajax({
            url: "<?php echo Class_config::get('urlApp') ?>/?controller=Pedidos&&action=obtieneTipoPlatoPantalla",
            method: 'POST',
    ...
    function cargarPedidos() {
        ...
        $.ajax({
            url: "<?php echo Class_config::get('urlApp') ?>/?controller=Pedidos&&action=obtienePedidosPantalla",
            data: {
                tipos: tipos
            },
            method: 'POST',
    ...
    function dibujarPedidos(pedidos_activos) {
        ...
        pedidos_actuales.forEach(pedido => {
            ...
            if ( i > -1) {
                if(salon == 0 || pedido['pkSalon'] == salon && pedido['estado'] == '1'){
                    let cliente = 'No definido';
                    if(pedido['cliente'] != null){
                        cliente = pedido['cliente'];
                    }else if(pedido['cliente_ext'] != null){
                        cliente = pedido['cliente_ext'];
                    }
                    ...
                    tempHtml += "<h3><b>CLIENTE: " + cliente + "</b></h3>";
        ...
        pedidos_anulados.forEach(pedido => {
            if(salon == 0 || pedido['pkSalon'] == salon && pedido['estado'] != '2'){
                let cliente = 'No definido';
                if(pedido['cliente'] != null){
                    cliente = pedido['cliente'];
                }else if(pedido['cliente_ext'] != null){
    ...
    function dibujarPedidosPorMesa(pedidos_activos){
        ...
        mesas.forEach(mesa => {
            if(salon == 0 || mesa.salon == salon){
                    ...
                    if(pedido["salon"]+" - "+pedido["mesa"] == mesa.mesa){
                        if(pedido['estado'] != '2'){
                            if(pedidos_activos.findIndex(x => x.id === pedido["id"]) > -1) {
                                ...
                                tempHtml2 += "<p style='font-size: 12px;' class='list-group-item-text'><b>CLIENTE: </b> "+cliente+"</p>";
                ...
                pedidos_anulados.forEach(pedido => {
                    if(pedido["salon"]+" - "+pedido["mesa"] == mesa.mesa && pedido['estado'] != '2'){
                        ...
                        tempHtml2 += "<p style='font-size: 12px;' class='list-group-item-text text-muted'><b>CLIENTE: </b> "+cliente+"</p>";
    ...
    function despachar(id) {
        ...
        $.ajax({
            url: "<?php echo Class_config::get('urlApp') ?>/?controller=Pedidos&&action=entregaPedidoPantalla",
            data: {
                id: id
            },
            method: 'POST',
    ...
    function cargarHistorial(){
        ...
        $.ajax({
            url: "<?php echo Class_config::get('urlApp') ?>/?controller=Pedidos&&action=obtieneHistorialDelDiaPantalla",
            data: {
                tipos: tipos,
                inicio: inicio,
                fin: numItems
    ...
    function actualiza_entrega(){
        if($('#check_entrega').is(":checked")) {
            $.ajax({
                url: "<?php echo Class_config::get('urlApp') ?>/?controller=Pedidos&&action=cookieImpresionPantalla",
                data: {
                    f: 'imp',
                    e: 1
                },
